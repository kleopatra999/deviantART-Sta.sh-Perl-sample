...
get '/auth/deviantart/callback' => sub {
   ...
   my $request = POST '',
     ...
     Content => [name => 'New Folder Name', folderid => '1234577899']; # use the folderid parameters from the result of a sta.sh submission

   my $response = $access_token->post('/api/draft15/stash/folder', $request->headers, $request->content);
